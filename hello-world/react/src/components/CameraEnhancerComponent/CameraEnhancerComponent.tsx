...
class CameraEnhancerComponent extends React.Component {
  ...
  async componentDidMount() {
    try {
      const cameraView = await (this.pInstance = CameraView.createInstance(
        this.elRef.current!
      ));
      // Should judge if 'cameraView' is disposed after 'await', as in development React runs setup and cleanup one extra time before the actual setup in Strict Mode.
      if (cameraView.disposed) return;
      const cameraEnhancer = await (this.pInstance =
        CameraEnhancer.createInstance(cameraView));
      // Should judge if 'cameraEnhancer' is disposed after 'await', as in development React runs setup and cleanup one extra time before the actual setup in Strict Mode.
      if (cameraView.disposed || cameraEnhancer.disposed) return;
  ...
  shouldComponentUpdate() {
    // Never update UI after mount, dcejs sdk use native way to bind event, update will remove it.
    return false;
  }
  render() {
    return (
      <div ref={this.elRef} className="component-camera-enhancer">
        ...
        <div className="dce-msg-poweredby">
...
export default CameraEnhancerComponent;
