...
let pInstance: Promise<CameraView | CameraEnhancer> | null = null;
...
onMounted(async () => {
  try {
    ...
    cameraView = await (pInstance = CameraView.createInstance(elRefs.value));
    cameraEnhancer = await (pInstance =
...
onBeforeUnmount(async () => {
  if (cameraView && !cameraView.disposed) cameraView.dispose();
  if (cameraEnhancer && !cameraEnhancer.disposed) cameraEnhancer.dispose();
  if (pInstance) {
    const instance = await pInstance;
...
<template>
  <div ref="elRefs" class="component-camera-enhancer">
